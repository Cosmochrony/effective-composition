...
\hypersetup{
  pdftitle={Finite Support and Preparation Criteria for Independent Effective Subsystems: A Contract for Non-Injective Admissibility},
...
\title{Finite Support and Preparation Criteria for Independent Effective Subsystems:\\
A Contract for Non-Injective Admissibility}
...
\begin{document}
  ...
  \begin{abstract}
    ...
    separate, checkable layers.
    In the finite case we characterise the possibilistic layer completely, and we resolve the preparation
    layer for two reference classes: the full simplex of preparations, and the strictly positive
    cylindrical-conditioning families; the general preparation layer remains open.
    ...
    preparations, every joint outcome law, in particular every product law, lifts to the substrate;
    ...
    but their realisation from the programme's axioms is not derived here, and the physical necessity of
    the contract itself is not derived from an operational theory.
    \emph{Interpretive reading (structural, not a result):} the absence of a supplied subsystem
  ...
  \subsection{The composition gap}
    ...
    Non-injectivity is structurally forced for genuine emergence~\cite{Beau2026l}, and a companion result
    shows that Bell-type correlations can be obtained from a many-to-one projection without any subsystem
    decomposition of the underlying configuration space~\cite{Beau2026b}; whether such correlations are a
    signature of non-injectivity is a separate question, since a projection can be non-injective through
    fibre multiplicity that is invisible to a factorising effective model.
  ...
  \subsection{A missing transversal contract}
    ...
    This paper characterises the support precondition of~\eqref{eq:k4} completely under the proposed
    contract, resolves its preparation precondition for two reference classes (the full simplex and the
    strictly positive cylindrical conditionings), leaves the general preparation layer open, and leaves
    open its states-and-effects layer: the construction of effect spaces, their bilinear pairing
  ...
  \subsection{Contributions}
    ...
    not derived from an operational theory.
  ...
  \subsection{The two composability tests}
    ...
    Within the proposed contract, independent composability of the two selections is tested by two
    properties, which we deliberately separate because they fail differently.
  ...
  \section{The Preparation Layer: Weights Decide What Support
  \texorpdfstring{\mbox{Cannot}}{Cannot}}
  ...
  \subsection{Consequences for the programme}
    ...
    to hold.
    In the vocabulary of~\eqref{eq:k4}: the support precondition of K4 is characterised completely under
    the proposed contract, its preparation precondition is resolved for the full simplex and for the
    strictly positive cylindrical-conditioning class while remaining open in general, its
    states-and-effects layer is untouched, and the corpus constructions that consume a
  ...
  \subsection{Interpretive outlook}
    ...
    a result.
    The absence of a supplied subsystem factorisation does not forbid independent effective subsystems.
  ...
  \label{app:repro}
    ...
    \begin{itemize}
      ...
      and the steered-margin variant violating the type discipline (five local states per side and
      seventeen failing pairs on $N_3$).
    \end{itemize}
